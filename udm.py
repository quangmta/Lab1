    ...
    def wrelf32(self, base_offset, filename):
        ...
        try:
            ...
            for h in range(e_phnum[0]):
                ...
                print("%2d" % phnum, "| 0x%08x" % prog_header[0], "| 0x%08x" % prog_header[1],
                      "| 0x%08x" % prog_header[2], "| 0x%08x" % prog_header[3], "| 0x%08x" % prog_header[4],
                      "| 0x%08x" % prog_header[5], "| 0x%08x" % prog_header[6], "| 0x%08x" % prog_header[7])
    ...
    def memtest32(self, baseaddr, wsize):
        ...
        for i in range(wsize):
            if (rddata[i] != wrdata[i]):
                print("memtest32 failed on address ", hex(baseaddr + (i << 2)), "expected data: ", hex(wrdata[i]),
                      " data read: ", hex(rddata[i]))
                test_succ = False
